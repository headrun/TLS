...
class nulled(scrapy.Spider):
    ...
    def __init__(self):
        self.conn =  MySQLdb.connect(db= "nulled", host = "localhost", charset="utf8mb4")#MySQLdb.connect(db= "nulled", host = "127.0.0.1", user="root", passwd = "123", use_unicode=True, charset="utf8mb4")
	...
	meta = {'crawl_type':'keep_up'}
		...
		yield Request(url,callback = self.parse_forums_,headers=headers, meta=meta)
	...
	threads_id = response.xpath(nulled_xpath.threads_id_xpath).extract()
		...
		yield Request(next_page,callback = self.parse_forums_,headers=headers, meta=meta)
