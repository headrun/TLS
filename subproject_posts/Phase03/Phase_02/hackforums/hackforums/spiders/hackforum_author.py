...
class Hackforums(scrapy.Spider):
    ...
    def parse_details(self, response):
	 ...
	 reference_url = response.url
         ...
         try:
             join_date = datetime.datetime.strptime(joindates, '%m-%d-%Y')
             joindate = time.mktime(join_date.timetuple())*1000
         except:
             try:
                 if '{1}' in joindates:
                     join_date = datetime.datetime.now() - datetime.timedelta(1)
                     joindate = int((datetime.datetime.now() - datetime.timedelta(1)).strftime("%s")) * (1000)
             except:
                 joindate = 0
