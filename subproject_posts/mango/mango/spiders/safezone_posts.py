...
class Safezone1(scrapy.Spider):
	...
	sub_categoryurl = response.xpath('//li[@itemprop = "itemListElement"]//a//@href')[2].extract()
	if 'http' not in sub_categoryurl:
	    sub_category_url = 'https://safezone.cc' + sub_categoryurl
	if sub_categoryurl == '':
		...
		author_url = 'Null'
            text = ' '.join(node.xpath('.//div[@class = "bbWrapper"]//text() |.//div[@class="bbWrapper"]//img[contains(@data-src,"/data/attachments/")]//@alt | .//div[@class = "bbWrapper"]//img[@class="smilie"]//@alt |.//div[@class = "bbWrapper"]//img[contains(@src,".gif")]//@alt |.//div[@class="bbWrapper"]//img[contains(@src,"data:image/gif;")]//@alt  |.//h4[@class="block-textHeader"]//text() |.//blockquote[@class="bbCodeBlock bbCodeBlock--expandable bbCodeBlock--quote"]/@class').extract()).strip().replace('bbCodeBlock bbCodeBlock--expandable bbCodeBlock--quote','Quote').replace('\n', '').replace('\r', '').replace('\t', '')  or 'Null'
