...
class Hacker_threads(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for cate in categories[:2]:
            if "http" not in cate: cate = "https://www.hackerthreads.org/" + cate
    ...
    def parse_links(self, response):
        ...
        for link in links:
            if "http" not in link: link = "https://www.hackerthreads.org/" + link
            #link = "https://www.hackerthreads.org/Topic-45298"
        ...
        if nxt_pg:
            if "http" not  in nxt_pg: nxt_pg = "https://www.hackerthreads.org/" + nxt_pg
    ...
    def parse_meta(self, response):
        ...
        if len(url_) > 2:
            ...
            crawl_type = "keepup"
	json_posts = {}
        #thread_url = url.replace(re.sub('https:(.*)Topic-\d+','',url),'')
        thread_url = ''.join(re.findall('(.*)-',url))
        ...
        for node in nodes:
            ...
            if author_link:
                ...
                author_link = ''.join(re.findall('(.*)\&sid', author_link))
            Link = []
            ...
            #links = node.xpath('.//a[@class="postlink"]/@href | .//img[@class="postimage"]/@src').extract()
            #links = node.xpath('.//div[@class="content"]//a[@class="postlink"]/@href | .//div[@class="content"]//img[@class="postimage"]/@src').extract()
            ...
            if author_link:
                ...
                crawl_query = utils.generate_upsert_query_crawl('hacker_threads')
        ...
        if nxt_pg:
            ...
            if "http" not in next_pg: next_pg = site_domain + next_pg
