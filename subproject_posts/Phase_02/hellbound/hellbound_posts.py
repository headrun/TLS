...
AUTHOR_CRAWL_QUERY = utils.generate_upsert_query_crawl('hellbound')
...
class Hellbound(Spider):
    ...
    def start_requests(self):
        ...
        for url in data:
            url = url[0]
            yield Request(url, callback = self.parse_meta,meta = {'crawl_type':'keep up','post_url':url})

    def parse_meta(self, response):
        json_values = {}
        thread_url = response.meta.get('post_url','')
        try:
