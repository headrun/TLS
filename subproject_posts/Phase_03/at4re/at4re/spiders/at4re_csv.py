...
def get_conn():
    ...
    conn = MySQLdb.connect(user="root", host = "localhost", db="at4re", passwd="", use_unicode=True, charset = 'utf8')
    cursor=conn.cursor()
    return conn,cursor
...
class Data():
    ...
    file_name = "at4reposts_%s.csv"%(str(datetime.datetime.now().date()))
    ...
    excel_auth_name1 = "at4reauthor_%s.csv"%(str(datetime.datetime.now().date()))
