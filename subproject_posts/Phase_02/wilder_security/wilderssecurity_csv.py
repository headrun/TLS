...
def get_conn():
    conn=MySQLdb.connect(db="POSTS_WILDER",host="localhost",user="root",passwd="root", use_unicode=True,charset="utf8")
    cursor=conn.cursor()
    return conn,cursor
...
class Data():
    ...
    file_name = "TlS_Posts_wildersecurity_%s.csv"%(str(datetime.datetime.now().date()))
    ...
    excel_auth_name1 = "TLS_Author_wildersecurity_%s.csv"%(str(datetime.datetime.now().date()))
    ...
    query = 'SELECT * from wilder_posts'
    cursor.execute(query)
    row = cursor.fetchall()
    for i in row:
        todays_excel_file.writerow(i)
    que = 'SELECT * from wilder_authors'
    cursor.execute(que)
