...
def generate_upsert_query_authors(crawler):
    ...
    upsert_query = """ INSERT INTO {0} (user_name, domain, crawl_type, author_signature, join_date, last_active, \
                    total_posts, fetch_epoch, groups, reputation, credits, awards, rank, active_time, contact_info,\
                    reference_url) VALUES ( %(user_name)s, %(domain)s, %(crawl_type)s, %(author_signature)s, \
                    %(join_date)s, %(last_active)s, %(total_posts)s, %(fetch_epoch)s, %(groups)s, %(reputation)s,\
                    ...
                    fetch_epoch=%(fetch_epoch)s, groups=%(groups)s, reputation=%(reputation)s, credits=%(credits)s,\
...
def generate_upsert_query_posts(crawler):
    ...
    upsert_query = """INSERT INTO {0} (domain, crawl_type, category, sub_category, thread_title, thread_url, post_id,post_title,\
                    ...
                    category=%(category)s, sub_category=%(sub_category)s, post_title=%(post_title)s, \
                    ...
                    all_links=%(all_links)s, reference_url=%(reference_url)s, thread_title = %(thread_title)s""".format(table_name).decode("iso-8859-1").encode('cp1252')
