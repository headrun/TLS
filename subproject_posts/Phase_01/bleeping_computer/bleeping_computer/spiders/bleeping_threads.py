...
class BleepingSpider(scrapy.Spider):
    ...
    def parse_threads(self, response):
        ...
        if next_page_link:
	    try:
		post_url = ','.join(post_nodes[-1].xpath(xpaths.POST_URL).extract()).strip()
		last_post_id = re.findall('entry(\d+)', post_url)[0]
                que_ = 'select * from bleeping_posts  where post_id = %(post_id)s'
