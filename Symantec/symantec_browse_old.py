...
def driver_data(driver, connection, cursor, surl, crawl_type):
		...
		insert_query = "insert into symantec_browse(data_links,reference_url,crawl_type,created_at,modified_at,crawl_status) values(%s,%s,%s,now(),now(),0) on duplicate key update modified_at = now()"
