...
def start_process():
    ...
    try:
        cursor.execute('select distinct(url) from crawling_sources.Microsoft_terminal_table where crawl_status = 0 limit 5')
    ...
    for links in db_data:
        ...
        if description_link in link :
            ...
            res = es.index(index="malware_docs", doc_type='malwaredoc', id=hashlib.md5(link).hexdigest(), body=doc)
            upd_qry = 'update crawling_sources.Microsoft_terminal_table set crawl_status = 1 where url = "%s"'
            values = link
            cursor.execute(upd_qry%values)
            connection.commit()
